...
def pull_ollama_model(model_name):
    json_response = requests.post(url = f"http://localhost:11434/api/tags", json = {"model": model_name}).json()
    print(json_response)
...
def list_ollama_models():
    response = requests.get(url="http://localhost:11434/api/tags")
    json_response = response.json()  # This is the JSON you provided
    models = json_response.get("models", [])
    model_names = [model["name"] for model in models]  # Extracting model names
    return model_names

st.session_state.model_options = list_ollama_models()
st.write(st.session_state.model_options) 
